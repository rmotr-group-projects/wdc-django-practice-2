...
def artists(request):
    """
        ...
        Part 2:
            - Task 1: Add a new 'genre' CharField to the Artist model in models.py
            with some choices. Then add to this view a new genre filter from a
            'genre' GET param (if given), in a similar way to the tasks before.
            If genre param is given, filter the artists queryset only with
...
def songs(request, artist_id=None):
    """
        Part 3:
            - Task 1: After creating the new Song model with its migration,
